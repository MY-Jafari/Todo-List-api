...
def test_task_filtering():
    ...
    Task.objects.create(user=user, list=list1, task_title='Buy Milk', priority='low', status='todo')
    Task.objects.create(user=user, list=list1, task_title='Buy Bread', priority='medium', status='inprogress')
    Task.objects.create(user=user, list=list2, task_title='Read Book', priority='high', status='done')
